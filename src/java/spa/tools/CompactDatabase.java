 ...
 *
 *   java -cp "classes:lib/*:conf" spa.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Dnxt.runtime.mode=desktop spa.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize Spa properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "spa.testDb" : "spa.db";
        String dbType = Spa.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Spa.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Spa.getDbDir(Spa.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Spa.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Spa.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Spa.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "spa.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "spa.mv.db");
            if (!dbFile.exists()) {
