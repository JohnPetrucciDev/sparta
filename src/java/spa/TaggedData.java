...
import spa.db.VersionedPrunableDbTable;
import spa.db.VersionedValuesDbTable;
import spa.util.Logger;
import spa.util.Search;
...
public class TaggedData {
        ...
        protected void prune() {
            if (Constants.ENABLE_PRUNING) {
                try (Connection con = db.getConnection();
                    ...
                    int expiration = Spa.getEpochTime() - Constants.MAX_PRUNABLE_LIFETIME;
    ...
    public static final class Tag {
        ...
        private static void add(TaggedData taggedData) {
            for (String tagValue : taggedData.getParsedTags()) {
                ...
                if (tag == null) {
                    tag = new Tag(tagValue, Spa.getBlockchain().getHeight());
                }
                tag.count += 1;
                tagTable.insert(tag);
            }
    ...
    public TaggedData(Transaction transaction, Attachment.TaggedDataAttachment attachment) {
        this(transaction, attachment, Spa.getBlockchain().getLastBlockTimestamp(), Spa.getBlockchain().getHeight());
    }
    ...
    static void add(TransactionImpl transaction, Attachment.TaggedDataUpload attachment) {
        if (Spa.getEpochTime() - transaction.getTimestamp() < Constants.MAX_PRUNABLE_LIFETIME && attachment.getData() != null) {
    ...
    static void extend(Transaction transaction, Attachment.TaggedDataExtend attachment) {
        ...
        if (Spa.getEpochTime() - Constants.MAX_PRUNABLE_LIFETIME < timestamp.timestamp) {
            ...
            if (taggedData != null) {
                ...
                taggedData.blockTimestamp = Spa.getBlockchain().getLastBlockTimestamp();
                taggedData.height = Spa.getBlockchain().getHeight();
                taggedDataTable.insert(taggedData);
            }
        }
    }
