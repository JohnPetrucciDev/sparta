...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && Spa.getBlockchainProcessor().isScanning() ?
                    Math.max(Spa.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
                    : Spa.getBlockchainProcessor().getMinRollbackHeight();
    ...
    private boolean doesNotExceed(int height) {
        return Spa.getBlockchain().getHeight() <= height && ! (isPersistent() && Spa.getBlockchainProcessor().isScanning());
    }
