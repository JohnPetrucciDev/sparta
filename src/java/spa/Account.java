...
public final class Account {
    ...
    private static final VersionedEntityDbTable<AccountAsset> accountAssetTable = new VersionedEntityDbTable<AccountAsset>("account_asset", accountAssetDbKeyFactory) {
        ...
        public void checkAvailable(int height) {
            if (height + Constants.MAX_DIVIDEND_PAYMENT_ROLLBACK < Spa.getBlockchainProcessor().getMinRollbackHeight()) {
                throw new IllegalArgumentException("Historical data as of height " + height +" not available.");
            }
            if (height > Spa.getBlockchain().getHeight()) {
                throw new IllegalArgumentException("Height " + height + " exceeds blockchain height " + Spa.getBlockchain().getHeight());
            }
        }
    ...
    public long getEffectiveBalanceSPA(int height) {
        ...
        try {
            long effectiveBalanceAPL = getLessorsGuaranteedBalanceAPL(height);
            if (activeLesseeId == 0) {
                effectiveBalanceAPL += getGuaranteedBalanceAPL(Constants.GUARANTEED_BALANCE_CONFIRMATIONS, height);
            }
    ...
    private long getLessorsGuaranteedBalanceAPL(int height) {
        ...
        for (int i = 0; i < lessors.size(); i++) {
            ...
            balances[i] = lessors.get(i).getBalanceAPL();
        }
    ...
    void leaseEffectiveBalance(long lesseeId, int period) {
        int height = Spa.getBlockchain().getHeight();
