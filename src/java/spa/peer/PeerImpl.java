...
import spa.util.CountingInputReader;
import spa.util.CountingInputStream;
import spa.util.CountingOutputWriter;
...
final class PeerImpl implements Peer {
    ...
    void setVersion(String version) {
        ...
        isOldVersion = false;
        if (Spa.APPLICATION.equals(application)) {
    ...
    public int getWeight() {
        ...
        if (hallmarkBalance == -1 || hallmarkBalanceHeight < Spa.getBlockchain().getHeight() - 60) {
            long accountId = hallmark.getAccountId();
            Account account = Account.getAccount(accountId);
            hallmarkBalance = account == null ? 0 : account.getBalanceAPL();
            hallmarkBalanceHeight = Spa.getBlockchain().getHeight();
        }
        return (int)(adjustedWeight * (hallmarkBalance / Constants.ONE_SPA) / Constants.MAX_BALANCE_SPA);
    }
    ...
    public void blacklist(Exception cause) {
        if (cause instanceof SpaException.NotCurrentlyValidException || cause instanceof BlockchainProcessor.BlockOutOfOrderException
    ...
    public void blacklist(String cause) {
        blacklistingTime = Spa.getEpochTime();
    ...
    public JSONObject send(final JSONStreamAware request, int maxResponseSize) {
        ...
        try {
            ...
            if (useWebSocket && !webSocket.isOpen())
                useWebSocket = webSocket.startClient(URI.create("ws://" + host + ":" + getPort() + "/spa"));
            //
            ...
            if (useWebSocket) {
                ...
                if (maxResponseSize > 0) {
                    ...
                    if (wsResponse.length() > maxResponseSize)
                        throw new SpaException.SpaIOException("Maximum size exceeded: " + wsResponse.length());
                ...
                //
                URL url = new URL("http://" + host + ":" + getPort() + "/spa");
                if (communicationLoggingMask != 0)
        ...
        } catch (SpaException.SpaIOException e) {
            blacklist(e);
            if (connection != null) {
                connection.disconnect();
            }
    ...
    void connect() {
        lastConnectAttempt = Spa.getEpochTime();
        try {
    ...
    boolean analyzeHallmark(final String hallmarkString) {
        ...
        try {
            ...
            for (PeerImpl peer : groupedPeers) {
                peer.adjustedWeight = Constants.MAX_BALANCE_SPA * peer.getHallmarkWeight(mostRecentDate) / totalWeight;
