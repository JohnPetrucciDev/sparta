...
public final class PopOff extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        try {
            Spa.getBlockchainProcessor().setGetMoreBlocks(false);
            if (numBlocks > 0) {
                blocks = Spa.getBlockchainProcessor().popOffTo(Spa.getBlockchain().getHeight() - numBlocks);
            } else if (height > 0) {
                blocks = Spa.getBlockchainProcessor().popOffTo(height);
            } else {
                return JSONResponses.missing("numBlocks", "height");
            }
        } finally {
            Spa.getBlockchainProcessor().setGetMoreBlocks(true);
        }
        ...
        if (keepTransactions) {
            blocks.forEach(block -> Spa.getTransactionProcessor().processLater(block.getTransactions()));
        }
        return response;
    }
