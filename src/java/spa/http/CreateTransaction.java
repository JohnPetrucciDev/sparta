...
import static spa.http.JSONResponses.FEATURE_NOT_AVAILABLE;
import static spa.http.JSONResponses.INCORRECT_DEADLINE;
import static spa.http.JSONResponses.INCORRECT_EC_BLOCK;
import static spa.http.JSONResponses.INCORRECT_LINKED_FULL_HASH;
import static spa.http.JSONResponses.INCORRECT_WHITELIST;
import static spa.http.JSONResponses.MISSING_DEADLINE;
import static spa.http.JSONResponses.MISSING_SECRET_PHRASE;
import static spa.http.JSONResponses.NOT_ENOUGH_FUNDS;

abstract class CreateTransaction extends APIServlet.APIRequestHandler {

    private static final String[] commonParameters = new String[]{"secretPhrase", "publicKey", "feeAPL",
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId, long amountAPL)
            throws SpaException {
        return createTransaction(req, senderAccount, recipientId, amountAPL, Attachment.ORDINARY_PAYMENT);
    }

    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
                                            long amountAPL, Attachment attachment) throws SpaException {
...
//        }
        ...
        long feeAPL = ParameterParser.getFeeAPL(req);
        ...
        if (ecBlockId != 0 && ecBlockId != Spa.getBlockchain().getBlockIdAtHeight(ecBlockHeight)) {
            return INCORRECT_EC_BLOCK;
        }
        if (ecBlockId == 0 && ecBlockHeight > 0) {
            ecBlockId = Spa.getBlockchain().getBlockIdAtHeight(ecBlockHeight);
        }
        ...
        try {
            Transaction.Builder builder = Spa.newTransactionBuilder(publicKey, amountAPL, feeAPL,
...
//            builder.appendix(prunableEncryptedMessage);
            ...
            } catch (SpaException.NotYetEncryptedException ignore) {}
