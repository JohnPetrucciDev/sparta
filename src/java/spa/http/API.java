...
import static spa.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static spa.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
import static spa.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
...
public final class API {
    ...
    public static final String adminPassword = Spa.getStringProperty("spa.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = Spa.getIntProperty("spa.maxAPIRecords");
    static final boolean enableAPIUPnP = Spa.getBooleanProperty("spa.enableAPIUPnP");
    public static final int apiServerIdleTimeout = Spa.getIntProperty("spa.apiServerIdleTimeout");
    public static final boolean apiServerCORS = Spa.getBooleanProperty("spa.apiServerCORS");
    ...
    static {
        List<String> disabled = new ArrayList<>(Spa.getStringListProperty("spa.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = Spa.getStringListProperty("spa.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = Spa.getStringListProperty("spa.allowedBotHosts");
        ...
        boolean enableAPIServer = Spa.getBooleanProperty("spa.enableAPIServer");
        if (enableAPIServer) {
            final int port = Spa.getIntProperty("spa.apiServerPort");
            final int sslPort = Spa.getIntProperty("spa.apiServerSSLPort");
            final String host = Spa.getStringProperty("spa.apiServerHost");
            disableAdminPassword = Spa.getBooleanProperty("spa.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = Spa.getBooleanProperty("spa.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(Spa.getUserHomeDir()).resolve(Paths.get(Spa.getStringProperty("spa.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(Spa.getStringProperty("spa.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(Spa.getStringProperty("spa.keyStoreType"));
                List<String> ciphers = Spa.getStringListProperty("spa.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = Spa.getStringProperty("spa.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{Spa.getStringProperty("spa.apiWelcomeFile")});
            }

            String javadocResourceBase = Spa.getStringProperty("spa.javadocResourceBase");
            ...
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/spa");
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(Spa.getIntProperty("spa.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));

            servletHolder = apiHandler.addServlet(APIProxyServlet.class, "/spa-proxy");
            ...
            if (!Spa.getBooleanProperty("spa.enableAPIServerGZIPFilter")) {
                gzipHandler.setExcludedPaths("/spa", "/spa-proxy");
            }
            gzipHandler.setIncludedMethods("GET", "POST");
            gzipHandler.setMinGzipSize(spa.peer.Peers.MIN_COMPRESS_SIZE);
            ...
            if (Spa.getBooleanProperty("spa.apiFrameOptionsSameOrigin")) {
