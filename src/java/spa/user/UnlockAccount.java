...
import static spa.user.JSONResponses.LOCK_ACCOUNT;
...
public final class UnlockAccount extends UserServlet.UserRequestHandler {
    ...
    JSONStreamAware processRequest(HttpServletRequest req, User user) throws IOException {
        ...
        if (account == null) {
            ...
            try (DbIterator<? extends Transaction> transactions = Spa.getTransactionProcessor().getAllUnconfirmedTransactions()) {
                while (transactions.hasNext()) {
                    ...
                    if (Arrays.equals(transaction.getSenderPublicKey(), accountPublicKey)) {
                        ...
                        myTransaction.put("sentAmountAPL", transaction.getAmountAPL());
                        if (accountId == transaction.getRecipientId()) {
                            myTransaction.put("receivedAmountAPL", transaction.getAmountAPL());
                        }
                        myTransaction.put("feeAPL", transaction.getFeeAPL());
            ...
            int blockchainHeight = Spa.getBlockchain().getLastBlock().getHeight();
            try (DbIterator<? extends Block> blockIterator = Spa.getBlockchain().getBlocks(accountId, 0)) {
                while (blockIterator.hasNext()) {
                    Block block = blockIterator.next();
                    if (block.getTotalFeeAPL() > 0) {
                        JSONObject myTransaction = new JSONObject();
                        ...
                        myTransaction.put("earnedAmountAPL", block.getTotalFeeAPL());
            ...
            try (DbIterator<? extends Transaction> transactionIterator = Spa.getBlockchain().getTransactions(accountId, (byte) -1, (byte) -1, 0, false)) {
