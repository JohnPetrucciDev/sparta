...
final class TransactionDb {
    ...
    static TransactionImpl loadTransaction(Connection con, ResultSet rs) throws SpaException.NotValidException {
        try {
            ...
            long amountAPL = rs.getLong("amount");
            long feeAPL = rs.getLong("fee");
            ...
            TransactionImpl.BuilderImpl builder = new TransactionImpl.BuilderImpl(version, null,
                    amountAPL, feeAPL, deadline, transactionType.parseAttachment(buffer, version))
                    .timestamp(timestamp)
        ...
        try {
            ...
            for (TransactionImpl transaction : transactions) {
                try (PreparedStatement pstmt = con.prepareStatement("INSERT INTO transaction (id, deadline, "
                    ...
                    pstmt.setLong(++i, transaction.getAmountAPL());
                    pstmt.setLong(++i, transaction.getFeeAPL());
