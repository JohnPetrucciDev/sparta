...
public final class Convert {
    ...
    public static long parseAccountId(String account) {
        ...
        if (account.startsWith("SPA1")) {
            return Convert.spaDecode(account);
        } else {
            return Long.parseUnsignedLong(account);
        }
    }

    public static String spaAccount(long accountId) {
        byte[] bytes = Convert.toBytes(accountId);
        String hex = Convert.toHexString(bytes);
        bytes = Convert.parseHexString("00" + hex);
        return "SPA" + Base58Check.bytesToBase58(bytes);
    }

    public static long spaDecode(String spaAddr) {
        try {
            byte[] bytes = Base58Check.base58ToBytes(spaAddr.substring(3));
            String hex = Convert.toHexString(bytes);
            bytes = Convert.parseHexString(hex.substring(2));
            BigInteger bigInteger = new BigInteger(1, new byte[] {bytes[7], bytes[6], bytes[5], bytes[4], bytes[3], bytes[2], bytes[1], bytes[0]});
            return bigInteger.longValue();
        } catch (Exception e) {
            Logger.logDebugMessage("Base58check decoding failed for " + spaAddr + ": " + e.toString());
    ...
    public static String readString(ByteBuffer buffer, int numBytes, int maxLength) throws SpaException.NotValidException {
        if (numBytes > 3 * maxLength) {
            throw new SpaException.NotValidException("Max parameter length exceeded");
        }
    ...
    public static long parseSPA(String nxt) {
        return parseStringFraction(nxt, 8, Constants.MAX_BALANCE_SPA);
    }
