...
/**
 * Handle logging for the Spa node server
 */
public final class Logger {
     ...
     * The spa/conf/logging-default.properties and spa/conf/logging.properties configuration
    ...
    static {
        ...
        System.setProperty("java.util.logging.manager", "spa.util.SpaLogManager");
        if (!(LogManager.getLogManager() instanceof SpaLogManager)) {
            System.setProperty("java.util.logging.manager",
                    (oldManager != null ? oldManager : "java.util.logging.LogManager"));
        }
        if (! Boolean.getBoolean("spa.doNotConfigureLogging")) {
            try {
                Properties loggingProperties = new Properties();
                Spa.loadProperties(loggingProperties, "logging-default.properties", true);
                Spa.loadProperties(loggingProperties, "logging.properties", false);
                Spa.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(Spa.class);
        enableStackTraces = Spa.getBooleanProperty("spa.enableStackTraces");
        enableLogTraceback = Spa.getBooleanProperty("spa.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof SpaLogManager) {
            ((SpaLogManager) LogManager.getLogManager()).spaShutdown();
        }
    }
