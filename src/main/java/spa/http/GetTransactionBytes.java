...
import static spa.http.JSONResponses.INCORRECT_TRANSACTION;
import static spa.http.JSONResponses.MISSING_TRANSACTION;
import static spa.http.JSONResponses.UNKNOWN_TRANSACTION;
...
public final class GetTransactionBytes extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        transaction = Spa.getBlockchain().getTransaction(transactionId);
        JSONObject response = new JSONObject();
        if (transaction == null) {
            transaction = Spa.getTransactionProcessor().getUnconfirmedTransaction(transactionId);
            if (transaction == null) {
                return UNKNOWN_TRANSACTION;
            }
        } else {
            response.put("confirmations", Spa.getBlockchain().getHeight() - transaction.getHeight());
        }
