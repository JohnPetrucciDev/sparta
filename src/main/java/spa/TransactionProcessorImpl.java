...
import spa.db.DbClause;
import spa.db.DbIterator;
import spa.db.DbKey;
import spa.db.EntityDbTable;
...
final class TransactionProcessorImpl implements TransactionProcessor {
    ...
    private static final boolean enableTransactionRebroadcasting = Spa.getBooleanProperty("spa.enableTransactionRebroadcasting");
    private static final boolean testUnconfirmedTransactions = Spa.getBooleanProperty("spa.testUnconfirmedTransactions");
    private static final int maxUnconfirmedTransactions;
    static {
        int n = Spa.getIntProperty("spa.maxUnconfirmedTransactions");
    ...
    private final Runnable removeUnconfirmedTransactionsThread = () -> {
        ...
        try {
            try {
                if (Spa.getBlockchainProcessor().isDownloading() && ! testUnconfirmedTransactions) {
                    return;
                }
                ...
                try (DbIterator<UnconfirmedTransaction> iterator = unconfirmedTransactionTable.getManyBy(
                        new DbClause.IntClause("expiration", DbClause.Op.LT, Spa.getEpochTime()), 0, -1, "")) {
    ...
    private final Runnable processTransactionsThread = () -> {
        ...
        try {
            try {
                ...
                } catch (SpaException.ValidationException|RuntimeException e) {
                    peer.blacklist(e);
                }
            } catch (Exception e) {
    ...
    @Override
    public void broadcast(Transaction transaction) throws SpaException.ValidationException {
        BlockchainImpl.getInstance().writeLock();
        try {
    ...
    @Override
    public void processPeerTransactions(JSONObject request) throws SpaException.ValidationException {
    ...
    void processWaitingTransactions() {
        ...
        try {
            if (waitingTransactions.size() > 0) {
                int currentTime = Spa.getEpochTime();
                ...
                while (iterator.hasNext()) {
                    ...
                    } catch (SpaException.ExistingTransactionException e) {
                        iterator.remove();
                    } catch (SpaException.NotCurrentlyValidException e) {
    ...
    private void processPeerTransactions(JSONArray transactionsData) throws SpaException.NotValidException {
        if (Spa.getBlockchain().getHeight() <= Constants.LAST_KNOWN_BLOCK && !testUnconfirmedTransactions) {
            return;
        }
        ...
        for (Object transactionData : transactionsData) {
            ...
            } catch (SpaException.NotCurrentlyValidException ignore) {
            } catch (SpaException.ValidationException|RuntimeException e) {
        ...
        if (!exceptions.isEmpty()) {
            throw new SpaException.NotValidException("Peer sends invalid transactions: " + exceptions.toString());
        }
    }

    private void processTransaction(UnconfirmedTransaction unconfirmedTransaction) throws SpaException.ValidationException {
        TransactionImpl transaction = unconfirmedTransaction.getTransaction();
        int curTime = Spa.getEpochTime();
        if (transaction.getTimestamp() > curTime + Constants.MAX_TIMEDRIFT || transaction.getExpiration() < curTime) {
            throw new SpaException.NotCurrentlyValidException("Invalid transaction timestamp");
        }
        if (transaction.getVersion() < 1) {
            throw new SpaException.NotValidException("Invalid transaction version");
        }
        if (transaction.getId() == 0L) {
            throw new SpaException.NotValidException("Invalid transaction id 0");
        }
        ...
        try {
            try {
                Db.db.beginTransaction();
                if (Spa.getBlockchain().getHeight() <= Constants.LAST_KNOWN_BLOCK && !testUnconfirmedTransactions) {
                    throw new SpaException.NotCurrentlyValidException("Blockchain not ready to accept transactions");
                }

                if (getUnconfirmedTransaction(transaction.getDbKey()) != null || TransactionDb.hasTransaction(transaction.getId())) {
                    throw new SpaException.ExistingTransactionException("Transaction already processed");
                }

                if (! transaction.verifySignature()) {
                    if (Account.getAccount(transaction.getSenderId()) != null) {
                        throw new SpaException.NotValidException("Transaction signature verification failed");
                    } else {
                        throw new SpaException.NotCurrentlyValidException("Unknown transaction sender");
                    }
                }

                if (! transaction.applyUnconfirmed()) {
                    throw new SpaException.InsufficientBalanceException("Insufficient balance");
                }

                if (transaction.isUnconfirmedDuplicate(unconfirmedDuplicates)) {
                    throw new SpaException.NotCurrentlyValidException("Duplicate unconfirmed transaction");
                }
     ...
     * @throws  SpaException.NotValidException    Transaction is not valid
     */
    @Override
    public List<Transaction> restorePrunableData(JSONArray transactions) throws SpaException.NotValidException {
