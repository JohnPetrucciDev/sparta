...
final class TransactionImpl implements Transaction {
    ...
    static final class BuilderImpl implements Builder {
        ...
        private final long amountAPL;
        private final long feeAPL;
        ...
        BuilderImpl(byte version, byte[] senderPublicKey, long amountAPL, long feeAPL, short deadline,
        ...
        @Override
        public TransactionImpl build(String secretPhrase) throws SpaException.NotValidException {
            if (timestamp == Integer.MAX_VALUE) {
                timestamp = Spa.getEpochTime();
            }
            if (!ecBlockSet) {
        ...
        @Override
        public TransactionImpl build() throws SpaException.NotValidException {
            return build(null);
        }
    ...
    private TransactionImpl(BuilderImpl builder, String secretPhrase) throws SpaException.NotValidException {

        this.timestamp = builder.timestamp;
        this.deadline = builder.deadline;
        this.senderPublicKey = builder.senderPublicKey;
        this.recipientId = builder.recipientId;
        this.amountAPL = builder.amountAPL;
        ...
        if (builder.feeAPL <= 0 || (Constants.correctInvalidFees && builder.signature == null)) {
            int effectiveHeight = (height < Integer.MAX_VALUE ? height : Spa.getBlockchain().getHeight());
            long minFee = getMinimumFeeAPL(effectiveHeight);
            feeAPL = Math.max(minFee, builder.feeAPL);
        } else {
            feeAPL = builder.feeAPL;
        }

        if (builder.signature != null && secretPhrase != null) {
            throw new SpaException.NotValidException("Transaction is already signed");
            ...
            if (getSenderPublicKey() != null && ! Arrays.equals(senderPublicKey, Crypto.getPublicKey(secretPhrase))) {
                throw new SpaException.NotValidException("Secret phrase doesn't match transaction sender public key");
            }
    ...
    @Override
    public long getAmountAPL() {
        return amountAPL;
    }

    @Override
    public long getFeeAPL() {
        return feeAPL;
    }
    ...
    byte[] bytes() {
        if (bytes == null) {
            try {
                ...
                if (useAPL()) {
                    buffer.putLong(amountAPL);
                    buffer.putLong(feeAPL);
                    ...
                    buffer.putInt((int) (amountAPL / Constants.ONE_SPA));
                    buffer.putInt((int) (feeAPL / Constants.ONE_SPA));
                    if (referencedTransactionFullHash != null) {
    ...
    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes) throws SpaException.NotValidException {
        try {
            ...
            long amountAPL = buffer.getLong();
            long feeAPL = buffer.getLong();
            ...
            TransactionImpl.BuilderImpl builder = new BuilderImpl(version, senderPublicKey, amountAPL, feeAPL,
            ...
            if (buffer.hasRemaining()) {
                throw new SpaException.NotValidException("Transaction bytes too long, " + buffer.remaining() + " extra bytes");
            }
            return builder;
    ...
    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes, JSONObject prunableAttachments) throws SpaException.NotValidException {
    ...
    public JSONObject getJSONObject() {
        ...
        json.put("amountAPL", amountAPL);
        json.put("feeAPL", feeAPL);
        if (referencedTransactionFullHash != null) {
    ...
    static TransactionImpl parseTransaction(JSONObject transactionData) throws SpaException.NotValidException {
        ...
        if (transaction.getSignature() != null && !transaction.checkSignature()) {
            throw new SpaException.NotValidException("Invalid transaction signature for transaction " + transaction.getJSONObject().toJSONString());
        }
        return transaction;
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(JSONObject transactionData) throws SpaException.NotValidException {
        try {
            ...
            long amountAPL = Convert.parseLong(transactionData.get("amountAPL"));
            long feeAPL = Convert.parseLong(transactionData.get("feeAPL"));
            ...
            if (transactionType == null) {
                throw new SpaException.NotValidException("Invalid transaction type: " + type + ", " + subtype);
            }
            TransactionImpl.BuilderImpl builder = new BuilderImpl(version, senderPublicKey,
                    amountAPL, feeAPL, deadline,
    ...
    private boolean checkSignature() {
        if (!hasValidSignature) {
            hasValidSignature = signature != null && Crypto.verify(signature, zeroSignature(getBytes()), getSenderPublicKey(), useAPL());
        }
    ...
    private int signatureOffset() {
        return 1 + 1 + 4 + 2 + 32 + 8 + (useAPL() ? 8 + 8 + 32 : 4 + 4 + 8);
    }

    private boolean useAPL() {
        return true;
    }
    ...
    @Override
    public void validate() throws SpaException.ValidationException {
        if (timestamp == 0 ? (deadline != 0 || feeAPL != 0) : (deadline < 1 || feeAPL <= 0)
                || feeAPL > Constants.MAX_BALANCE_APL
                || amountAPL < 0
                || amountAPL > Constants.MAX_BALANCE_APL
                || type == null) {
            throw new SpaException.NotValidException("Invalid transaction parameters:\n type: " + type + ", timestamp: " + timestamp
                    + ", deadline: " + deadline + ", fee: " + feeAPL + ", amount: " + amountAPL);
        }

        if (referencedTransactionFullHash != null && referencedTransactionFullHash.length != 32) {
            throw new SpaException.NotValidException("Invalid referenced transaction full hash " + Convert.toHexString(referencedTransactionFullHash));
        }

        if (attachment == null || type != attachment.getTransactionType()) {
            throw new SpaException.NotValidException("Invalid attachment " + attachment + " for transaction of type " + type);
        }

        if (! type.canHaveRecipient()) {
            if (recipientId != 0 || getAmountAPL() != 0) {
                throw new SpaException.NotValidException("Transactions of this type must have recipient == 0, amount == 0");
            }
        }

        if (type.mustHaveRecipient() && version > 0) {
            if (recipientId == 0) {
                throw new SpaException.NotValidException("Transactions of this type must have a valid recipient");
            }
        }

        boolean validatingAtFinish = false;
        for (Appendix.AbstractAppendix appendage : appendages) {
            appendage.loadPrunable(this);
            if (! appendage.verifyVersion(this.version)) {
                throw new SpaException.NotValidException("Invalid attachment version " + appendage.getVersion()
        ...
        if (getFullSize() > Constants.MAX_PAYLOAD_LENGTH) {
            throw new SpaException.NotValidException("Transaction size " + getFullSize() + " exceeds maximum payload size");
        }

        if (!validatingAtFinish) {
            int blockchainHeight = Spa.getBlockchain().getHeight();
            long minimumFeeAPL = getMinimumFeeAPL(blockchainHeight);
            if (feeAPL < minimumFeeAPL) {
                throw new SpaException.NotCurrentlyValidException(String.format("Transaction fee %f SPA less than minimum fee %f SPA at height %d",
                        ((double) feeAPL) / Constants.ONE_SPA, ((double) minimumFeeAPL) / Constants.ONE_SPA, blockchainHeight));
            }
        }
    ...
    void apply() {
        ...
        if (referencedTransactionFullHash != null
            ...
            senderAccount.addToUnconfirmedBalanceAPL(getType().getLedgerEvent(), getId(),
                    0, Constants.UNCONFIRMED_POOL_DEPOSIT_APL);
        }
        if (attachmentIsPhased()) {
            senderAccount.addToBalanceAPL(getType().getLedgerEvent(), getId(), 0, -feeAPL);
        }
        for (Appendix.AbstractAppendix appendage : appendages) {
    ...
    private long getMinimumFeeAPL(int blockchainHeight) {
        long totalFee = 0;
        for (Appendix.AbstractAppendix appendage : appendages) {
            appendage.loadPrunable(this);
        ...
        if (referencedTransactionFullHash != null) {
            totalFee = Math.addExact(totalFee, Constants.ONE_SPA);
        }
        return totalFee;
    }
